...
async function entryEngine(jwt, fromdate, todate, futureToken) {
    ...
    try {
        ...
        if (atm) {
            ...
            optionSymbol = isPE
                ? `SENSEX${atm.strike}PE`
                : `SENSEX${atm.strike}CE`;
            ...
            // ── Fetch option LTP
            await sleep(300);
            const ltpData = await getLTP(jwt, {
                BFO: [optionToken]
            });
...
async function main() {
    ...
    while (true) {
        ...
        try {
            ...
            // ✅ FIX 1: today's 09:15 only (no daysBack)
            const liveFrom = getTodayFromDate();
            // ✅ FIX 2: live current time, not hardcoded
            ...
            // ✅ FIX 3: signalObj is now an object, not a string
