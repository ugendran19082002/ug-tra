...
async function backtest(index1mAll, future1mAll, data1D, options = {}) {
    const {
        slPoints = 80,
        tgtPoints = 200,
        tslTrigger = 0,   // when unrealised profit reaches this → activate TSL
        tslLockIn = 0,    // SL moves to entryPrice ± this value (lock-in profit)
        useTSL = true,
        startBar = 30,
    ...
    trades.forEach((t, idx) => {
        ...
        btLogger.info(
            ...
            pnlStr.padEnd(12) +
            t.exitReason.padEnd(10) +
            String(t.exitBar - t.entryBar).padEnd(12) +
...
async function main() {
    ...
    if (isBacktest) {
        ...
        await backtest(index1m, future1m, data1D, {
            ...
            tgtPoints: parseInt(process.env.BT_TGT ?? "400"),
            startBar: 30,
        });
