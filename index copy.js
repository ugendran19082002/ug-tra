...
// ─────────────────────────────────────────
async function getOIData(jwt, token, interval = "FIVE_MINUTE", retries = 3) {
    const body = {
        "exchange": "BSE",
        ...
        // todate:   formatDateTime()   // ✅ uncomment for live
    };

    try {
        logger.debug(`📊 Fetching OI | token: ${token} | interval: ${interval}`);
        ...
        console.log("📈 Candles Count:", res.data.data || 0);

        if (res.data.data?.length) {
            console.log("🟢 First:", res.data.data[0]);
            console.log("🔵 Last :", res.data.data.at(-1));
        }
...
// ─────────────────────────────────────────
// ─────────────────────────────────────────
// OI CONFIRMATION  (uses dedicated OI endpoint)
// Price ↑ OI ↑ → Long Build-up   → BULLISH ✅
// Price ↓ OI ↑ → Short Build-up  → BEARISH ✅
// Price ↑ OI ↓ → Short Covering  → WEAK
// Price ↓ OI ↓ → Long Unwinding  → WEAK
...
function analyzeOI(future1m, oiData) {
    ...
    // ── Latest price from futures
    const currPrice = future1m[fLen - 1].close;
    const prevPrice = future1m[fLen - 5].close;

    // ── Latest OI from dedicated endpoint
    const currOI = oiData[len - 1].oi;
    const prevOI = oiData[len - 5].oi;
    ...
    logger.info(`━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`);
    logger.info(`📊 OI Analysis`);
    logger.info(`   Price : ${prevPrice} → ${currPrice} (${priceDiff > 0 ? "+" : ""}${priceDiff})`);
    logger.info(`   OI    : ${prevOI}    → ${currOI}    (${oiDiff > 0 ? "+" : ""}${oiDiff})`);
...
async function entryEngine(index1m, future1m, data1D, oiData) {
    ...
    const breakUp = last1m.high > prev1m.high;
    const breakDown = last1m.low < prev1m.low;
    ...
    const oi = analyzeOI(future1m, oiData);   // ✅ pass oiData
    ...
    if (
        ...
        volConfirm && oiConfirmsBear              // ✅ OI must show Short Build-up
...
`;
    ...
    if (
        ...
        (trendUp || bigCandle) &&
        (breakUp || breakAbove) &&
        volConfirm && oiConfirmsBull              // ✅ OI must show Long Build-up
...
async function main() {
    ...
    while (true) {
        ...
        try {
            ...
            // ── Validate
            if (!indexRaw.length || !futureRaw.length || !oiRaw.length) {
