...
async function getFutureToken(symbolName = "SENSEX", refDate = new Date()) {
    ...
    // Cache only valid for live mode (refDate = today)
    ...
    // ✅ CHANGE 1 — Use refDate instead of today for contract selection
    ...
    // Only cache to .env in live mode — don't overwrite with backtest token
    if (isLive) {
...
async function backtest(index1mAll, future1mAll, data1D, options = {}) {
    ...
    for (let i = startBar; i <= endBar; i++) {
        ...
        // ✅ BONUS — String compare (YYYY-MM-DD HH:MM format sorts correctly, avoids timezone bugs)
...
async function main() {
    ...
    if (isBacktest) {
        ...
        // ✅ CHANGE 1 — Pass btFrom so contract selection matches backtest period, not today
        ...
        // ✅ CHANGE 2 — Align by timestamp (not blind length slice)
        // Prevents missing half-days, gap days, or any timing mismatch between index & future
